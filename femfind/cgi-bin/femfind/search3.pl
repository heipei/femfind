...
<tr bgcolor="#99CCFF">
<td valign="top" class="b"><b>Results for [$_]</b></td>
</tr>
<tr bgcolor="#dddddd">
...
if ($c{hits} == $c{num_hits_displayed})
{
	$c{control} .= $q->a({-href => "$c{url}&offset=$c{offset}&index=$c{num_last}&fskip=$c{file_skip}&form=$c{form}"}, "More..."). " | ";
}
$c{control} .= $q->a( {-href => "/cgi-bin/femfind/frontpage.pl?form=$c{form}"}, "New search" );

$c{num_last}--;
my $treffer;
$treffer = "Hits $c{num_first} - $c{num_last}:" if ($c{hits} > 1);
$treffer = "Hits $c{num_first}:" if ($c{hits} == 1);
$treffer = "No hits." if ($c{hits} == 0);
